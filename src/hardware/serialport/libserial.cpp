...
bool SERIAL_open(const char* portname, COMPORT* port) {
	int result;
	printf("Running SERIAL_open for port name [%s]\n", portname);
	// allocate COMPORT structure
	COMPORT cp = (_COMPORT*)malloc(sizeof(_COMPORT));
	if(cp == NULL) return false;
    ...
    printf("Setting serial port attributes (agian?).\n");
...
bool SERIAL_setCommParameters(COMPORT port,
    ...
    printf("Setting requested baudrate to %d\n", baudrate);
    switch(baudrate) {
    ...
    case    200: posix_baudrate = B200; break;
    case    150: posix_baudrate = B150; break;
    case    134: posix_baudrate = B134; break;
    case    110: posix_baudrate = B110; break;
    case     75: posix_baudrate = B75; break;
    case     50: posix_baudrate = B50; break;
    default    : posix_baudrate = BNONE; break;
    }
    if(posix_baudrate != BNONE) {
    printf("     Standard (POSIX) baud rate detected.\n");
	...
	if(retval==-1){
	    ...
	    printf("     Custom baud rate detected\n");
	    int r = cfsetospeed_custom(&termInfo, baudrate);
	    if(r < 0) {
	        printf("Could not set baud rate to %d\n", baudrate);
	        return false;
	    }
	    printf("     Successfully set baud rate.\n");
	    return true;
	}
	
//customBaudrate:
//    termInfo->c_cflag = B38400;
//    termInfo->alt_speed = baudrate;
